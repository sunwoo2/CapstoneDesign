...
// Drive
	...
	// go		: 
	// stop	 	: 1200
    unsigned int left = 1200;     
    unsigned int straight = 1530; 
    unsigned int right = 1800;    
    unsigned int go = 1250;  
    unsigned int stop = 1200; 

    sleep(2);
	// 처음에 1초 쌔게 주고
    if( !gpioPWM(ESC, 1270) ) printf("good\n");
		sleep(2);
    if( !gpioPWM(ESC, go) ) printf("good\n");
    if( !gpioPWM(SERVO, straight) ) printf("good\n");
    sleep(2);	// Finish drive
//
