...
// Drive
	...
	unsigned int stop_start = 0, count_zero = 0;
...
//					} else if( cur_angle > 358 || cur_angle < 3 ) {
					} else if( cur_angle < 1.0f && cur_angle > 0.0f ) {
                		if( nodes[pos].sync_quality ) {
							cur_dist0 = (pre_dist0 + nodes[pos].distance_q2/4.0f)/2;
						} else {
							cur_dist0 = pre_dist0;
						}
						cur_dist0 = fmod(cur_dist0, 10000);
...
/* Only sensing difference of distance

		if( diff >= 10 || diff <= -10 ) {
			setpoint += (int)(diff*0.1);
			...
			//printf("%d \n", val);
    		if( !gpioPWM(SERVO, setpoint) ) {
				//(setpoint < 1540) ? printf("Left %d \n", setpoint) : printf("Right %d \n", setpoint);
				(diff < 0) ? printf("\n==== Left %d \n\n", setpoint) : printf("\n==== Right %d \n\n", setpoint);
			}
		} else {
			//printf("Same distance! \n");

			setpoint += (STRAIGHT - setpoint)*0.5;

    		if( !gpioPWM(SERVO, setpoint) ) printf("\n==== Straight! %d \n\n", setpoint);
		}
*/
...
// STOP  Algorithm
		...
		if( stop_start > 80 ) {
			if( cur_dist0 < 1500 && cur_dist0>10) {
				//count_zero++;
				//if( count_zero > 0 ) {
				    drv->stop();
				    drv->stopMotor();
